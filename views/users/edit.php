...
<html lang="pt-br">
  <head>
    ...
    <title>
      Edição de Usuário - André Lucas Trevizan Portfolio
    </title>
  </head>
  <body>
    ...
    <div class="container py-3 d-flex flex-column">
      ...
      <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2">
        <h2 class="mb-0">Editando Usuário</h2>
      ...
      <div>
        ...
        <form
          action="../../actions/updateUser.php"
          method="POST"
          class="d-flex flex-column gap-4 w-100"
        >
          <input name="user_id" type="text" hidden value="<?= $userDetails['id'] ?>">
          <div>
            <label class="form-label">Nome</label>
            <input type="text" class="form-control" name="user_name" placeholder="Entre com o nome do usuário" required value="<?= $userDetails['user_name'] ?>">
          </div>
          <div>
            <label class="form-label">E-mail</label>
            <input type="email" class="form-control" name="user_email" placeholder="Entre com o email do usuário" required value="<?= $userDetails['user_email'] ?>">
          </div>
          <div>
            <label class="form-label">Data de Nascimento</label>
            <input type="date" class="form-control" name="user_birth_date"  required value="<?= date('Y-m-d', strtotime($userDetails['user_birth_date'])) ?>">
          </div>
          <div>
            <label class="form-label">Descrição</label>
            <textarea name="user_description" class="form-control" rows="5" placeholder="Entre com uma descrição para o usuário" required><?= $userDetails['user_description'] ?></textarea>
          </div>
          <div>
            <label class="form-label">Função</label>
            <select class="form-select" aria-label="Default select example" required name="user_role" value="<?= $userDetails['user_role'] ?>">
              <option>Escolha uma função para o usuário</option>
              <option
                value="admin"
                <?= $userDetails['user_role'] === 'admin' ? 'selected' : '' ?>
              >
                Administrador
              </option>
              ...
              <option
                value="viewer"
                <?= $userDetails['user_role'] === 'viewer' ? 'selected' : '' ?>
              >
                Visualizador
              </option>
            </select>
